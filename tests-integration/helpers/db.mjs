...
export async function createTestUser(client, { superAdmin = false } = {}) {
  userCounter += 1;
  // N-1: `node --test` runs multiple *.test.mjs files concurrently, each in
  // its own module instance with its own independent `userCounter` starting
  // at 0 — so `Date.now()-userCounter` alone can collide between two files
  ...
  // fault-injection.test.mjs). The `randomUUID()` suffix makes this
  // collision-proof regardless of how many files/processes call this
  // concurrently.
  const email = `fault-injection-${Date.now()}-${userCounter}-${randomUUID()}@example.test`;
